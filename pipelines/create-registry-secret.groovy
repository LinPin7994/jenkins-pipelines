    ...
    stage("Create docker registry secret") {
        withCredentials([usernamePassword(credentialsId: 'nexusHTTP', passwordVariable: 'NEXUS_PASSWORD', usernameVariable: 'NEXUS_USERNAME')]) {
            ...
            if (checkSecretExist.length() > 0) {
                println("[JENKINS][DEBUG] Secert " + checkSecretExist + " exist.")
            } else {
                println("[JENKINS][DEBUG] Secret " + checkSecretExist + " doesn't exist and will be created.")
                sh(script: "kubectl --kubeconfig ${env.KUBECONFIG}  -n ${env.NAMESPACE} create secret docker-registry ${env.SECRET_NAME} --docker-server=${env.DOCKER_SERVER}:${env.DOCKER_SERVER_PORT} --docker-username=${NEXUS_USERNAME} --docker-password=${NEXUS_PASSWORD} --docker-email=${env.DOCKER_EMAIL}")
            }
        }
    }
}